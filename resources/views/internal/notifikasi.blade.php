...
<div x-data="notifPage()" x-init="init()" class="flex flex-col gap-4">
    ...
    {{-- Header Card --}}
    <div class="bg-white shadow-sm rounded-2xl px-4 md:px-6 py-4 flex items-center justify-between gap-2">
        <div class="min-w-0">
            <p class="text-xs md:text-sm text-gray-500">
                Anda memiliki <strong x-text="unreadCount" class="text-red-500"></strong> notifikasi belum dibaca.
            </p>
        </div>
        <button @click="markAllRead()"
                class="flex items-center justify-center gap-1.5 md:gap-2 px-3 md:px-4 py-1.5 md:py-2 text-xs font-semibold text-[#1a237e] border border-[#1a237e]/30 rounded-xl hover:bg-[#1a237e]/5 transition-colors shrink-0 whitespace-nowrap">
    ...
    <div class="bg-white shadow-sm rounded-2xl overflow-hidden">
        ...
        <div class="divide-y divide-gray-50 max-h-[60vh] overflow-y-auto">
            {{-- Date Group Headers --}}
            <template x-for="(group, gIndex) in groupedNotifications" :key="gIndex">
                <div>
                    <div class="sticky top-0 z-10 bg-white/90 backdrop-blur-sm px-4 md:px-6 py-2 border-b border-gray-100">
                        <p class="text-xs font-bold text-gray-400 uppercase tracking-wider" x-text="group.label"></p>
                    </div>
                    <template x-for="notif in group.items" :key="notif.id">
                        <div @click="markRead(notif.id)"
                             :class="notif.read ? '' : 'bg-blue-50/40'"
                             class="px-4 md:px-6 py-3 md:py-4 hover:bg-gray-50 transition-colors cursor-pointer group">
...
{{-- Empty State --}}
<div x-show="!groupedNotifications.length" class="px-4 md:px-6 py-16 text-center">
...
{{-- Footer --}}
<div class="px-4 md:px-6 py-3 border-t border-gray-100 bg-gray-50 flex items-center justify-between">
<p class="text-xs text-gray-400">
    Menampilkan <strong x-text="groupedNotifications.reduce((sum, g) => sum + g.items.length, 0)"></strong> notifikasi
</p>
</div>
    </div>
</div>
    ...
    return {
        ...
        get filteredNotifications() {
            ...
            if (this.activeTab === 'unread') {
                list = this.notifications.filter(n => !n.read);
            }
            return list;
        },
        ...
        get groupedNotifications() {
            ...
            Object.keys(map).sort((a, b) => map[b].date - map[a].date).forEach(key => {
                const g = map[key];
                const d = g.date;
                let label;
                ...
                groups.push({ label, items: g.items });
            });
        ...
        async loadNotifications() {
            ...
            try {
                ...
                this.notifications = (data.notifications || []).map(n => {
                    const d = new Date(n.datetime);
                    n.time = d.toLocaleTimeString('id-ID', { hour: '2-digit', minute: '2-digit' });
                    return n;
                });
